...
function attach(server) {
  ...
  server.get('/forecast', function (req, res, next) {
    ...
    layers = layers.map(function (layer) {
      var _layer$split = layer.split('@'),
          ...
          name = _layer$split2[0],
          surface = _layer$split2[1];

      return { name: name, surface: surface };
    });

    var fromDate = new Date(parseInt(req.query.from));
    var lat = clamp(req.query.lat, -90, 90);
    var lng = clamp(req.query.lng, -180, 180);
  ...
  server.get('/layer/:name/:date', function (req, res, next) {
    ...
    _GridLoader2.default.fetchGeoJSON(req.params.name, forecastedDate, bounds).then(function (geoJSON) {
      var buff = _geobuf2.default.encode(geoJSON, new _pbf2.default());
