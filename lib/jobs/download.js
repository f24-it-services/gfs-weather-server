...
function install(schedule) {
  schedule('* * * * *', function () {
    (0, _db.bootstrap)();
    downloaderConfig = _Config2.default.get().downloader;
    ...
    var start = function start(startDate) {
      var config = Object.assign({}, downloaderConfig, { fields: [] });
      downloaderConfig.fields.forEach(function (field) {
    ...
    if (_yargs.argv.date) {
      ...
      if (isNaN(date.getTime())) {
        return console.error('Invalid date ' + _yargs.argv.date);
      }
      promise = start(date);
    } else {
      //
