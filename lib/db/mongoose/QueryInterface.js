  ...
  _createClass(MongooseQueryInterface, [{
    ...
    value: function findPointsInBounds(dsCriteria, layerCriteria, bounds) {
      ...
      var pointCriteria = {
        $geoWithin: {
          $geometry: {
            type: 'Polygon',
            coordinates: [[[bounds[0], bounds[1]], [bounds[0], bounds[3]], [bounds[2], bounds[3]], [bounds[2], bounds[1]], [bounds[0], bounds[1]]]]
          }
        }
      };
    ...
    value: function cleanupOldDataSets(ttl) {
      ...
      return this.db.DataSet.find({
        ...
        return _this4.db.Point.remove({
          layer: { $in: layerIds }
        }).then(function () {
          return _this4.db.Layer.remove({ _id: { $in: layerIds } });
        }).then(function () {
          return _this4.db.DataSet.remove({ _id: { $in: dsIds } });
        });
      });
    }
  }]);
